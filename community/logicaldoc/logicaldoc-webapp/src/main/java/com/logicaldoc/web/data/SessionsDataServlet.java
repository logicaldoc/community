	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		try {
			...
			if (request.getParameter("kill") != null) {
				...
				System.out.println("Killed session " + request.getParameter("kill"));
				...
				if (currentUser != null && !mDao.isReadEnable(Menu.ADMIN_SESSIONS, currentUser.getId()))
					throw new Exception("User " + currentUser.getUsername()
							+ " doesn't have permission to access the sessions list");
				...
				if (csvFormat) {
					...
					for (Session session : sessions) {
						...
						SessionManager.get().get(session.getSid());

						writer.print("<session>");
						writer.print("<sid><![CDATA[" + session.getSid() + "]]></sid>");
						...
						writer.print("<node><![CDATA[" + session.getNode() + "]]></node>");
						...
						if (SessionManager.get().get(session.getSid()) != null)
							writer.print("<renew>"
									+ df.format(SessionManager.get().get(session.getSid()).getLastRenew()) + "</renew>");
						else
