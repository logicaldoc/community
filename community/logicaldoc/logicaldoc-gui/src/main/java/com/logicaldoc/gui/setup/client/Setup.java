...
public class Setup implements EntryPoint {
	...
	private Tab setupDatabase(final ValuesManager vm, GUIInfo info) {
		...
		dbType.setValue("hsqldb".equals(info.getConfig("jdbc.dbms")) ? I18N.message(EMBEDDED) : I18N
				.message("external"));
		...
		for (String engine : engines.keySet())
			valueMap.put(engine, MYSQL.equalsIgnoreCase(engine) ? "MySQL / MariaDB" : engines.get(engine)[0]);
