 ...
 * @since 7.3
 */
public class UpdatePanel extends AdminPanel {
	...
	private VLayout prepareDownloadProgress(final GUIParameter[] parameters) {
				...
				SC.ask(I18N.message("confirmupdate"), I18N.message("confirmupdatequestion"), new BooleanCallback() {
					...
					public void execute(Boolean choice) {
						if (choice.booleanValue()) {
							...
							UpdateService.Instance.get().confirm(updateFileName, new AsyncCallback<String>() {
		...
		download.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				UpdateService.Instance.get().download(Session.get().getInfo().getUserNo(),
						Util.getValue("id", parameters), updateFileName,
	...
	private void displayUpdateNotes(String fileName) {
		UpdateService.Instance.get().getNotes(fileName, new AsyncCallback<String[]>() {
