...
public class HibernateMenuDAOTest extends AbstractCoreTestCase {
    ...
    public void testDelete() throws PersistenceException {
        ...
        Menu menu = testSubject.findById(99L);
        assertNull(menu);

        DocumentDAO.get().delete(1L);
    ...
    public void testRestore() throws PersistenceException {
        assertEquals(1000L, testSubject
                .queryForLong("select ld_id from ld_menu where ld_id = %d and ld_deleted = 1".formatted(1000L)));
        assertEquals(1100L, testSubject
                .queryForLong("select ld_id from ld_menu where ld_id = %d and ld_deleted = 1".formatted(1100L)));
