...
public class SequencesWorkbench {
    ...
    public static void main(String[] args) {
        System.out.println("total tables: %d".formatted(tables.size()));
...
//		createPostreSQL();
    ...
    private static void createPostreSQL() {
        ...
        for (String table : tables) {
            System.out.println(
                    "create sequence %s_SEQ  increment by 50 start with START_%s_SEQ;".formatted(table, table));
        }
    ...
    private static void createOracle() {
        ...
        for (String table : tables) {
            System.out.println(
                    "create sequence %s_SEQ  start with START_%s_SEQ increment by 50;".formatted(table, table));
        }
        ...
        for (String table : tables) {
            System.out.println("""
                               ...
                               DECLARE
                                 rst bigint
                               BEGIN""");
            System.out.println(" select coalesce(max(ld_id),0) + 50  into rst from %s;".formatted(table));
            System.out.println("  alter sequence %s_SEQ RESTART WITH rst;".formatted(table));
            System.out.println("""
                               END;
                               $$""");
            System.out.println("");
    ...
    private static void resetOracle() {
        ...
        for (String table : tables) {
            ...
            System.out.println("  select nvl(max(ld_id),0) + 50  into rst from %s;".formatted(table));
            System.out.println("  alter sequence %s_SEQ RESTART START WITH rst;".formatted(table));
    ...
    private static void resetSqlServer() {
        ...
        for (String table : tables) {
            ...
            System.out.println("select @rst = isnull(max(ld_id),0) + 50  from %s;".formatted(table));
            System.out.println("alter sequence %s_SEQ RESTART WITH @rst;".formatted(table));
            System.out.println("");
        }
    ...
    private static void resetHSQLDB() {
        ...
        for (String table : tables) {
            ...
            System.out.println("select ifnull(max(ld_id),0) + 50 from %s;".formatted(table));
            System.out.println("alter sequence %s_SEQ RESTART WITH *{rst};".formatted(table));
            System.out.println("");
        }
    ...
    private static void resetMariaDB() {
        ...
        for (String table : tables) {
            System.out.println("select ifnull(max(ld_id),0) + 50  into @rst from %s;".formatted(table));
            System.out.println(
                    "EXECUTE IMMEDIATE CONCAT('alter sequence %s_SEQ RESTART WITH ', (@rst));".formatted(table));
        }
    ...
    private static void resetMySQL() {
        ...
        for (String table : tables)
            System.out.println("delete from %s_SEQ;");
        System.out.println(" ");
        for (String table : tables)
            System.out.println("insert into %s_SEQ select max(next_val) + 50 from ld_hilo where ld_sequence= '%s';"
                    .formatted(table));
