...
public class TagSearch extends Search {
    ...
    private void appendWhereClause(boolean aliases, StringBuilder query) throws PersistenceException {
        ...
        query.append("""
                     ...
                     """.formatted(AbstractDocument.NATURE_DOC, tenantId, DocumentStatus.ARCHIVED.ordinal()));

        if (CollectionUtils.isNotEmpty(getOptions().getFilterIds()))
            query.append(" and A.ld_id in (%s)".formatted(getOptions().getFilterIds().stream()
        ...
        List<Long> precoll = DocumentDAO.get().findDocIdByUserIdAndTag(options.getUserId(), options.getExpression());
        if (CollectionUtils.isNotEmpty(precoll))
            query.append(precoll.stream().map(id -> Long.toString(id)).collect(Collectors.joining(",")));
        else
            query.append("0");
        query.append(")");
