...
public class HibernateSearchDAO extends HibernatePersistentObjectDAO<SavedSearch> implements SearchDAO {
    ...
    public SavedSearch findByUserIdAndName(long userId, String name) throws PersistenceException {
        List<SavedSearch> searches = findByWhere("_entity.userId = :userId and _entity.name = :name",
    ...
    public void delete(long id, int code) throws PersistenceException {
        ...
        if (search != null) {
            ...
            search.setName("%s.%d".formatted(search.getName(), search.getId()));
            saveOrUpdate(search);
        }
    }
    ...
    private void setUniqueName(SavedSearch search) throws PersistenceException {
        ...
        queryForResultSet(
                ...
                Map.of(USERID, search.getUserId(), "baseName", "%s%%".formatted(baseName.toLowerCase()), "id",
                        search.getId()),
                null, rs -> {
                    while (rs.next()) {
        ...
        while (names.contains(search.getName().toLowerCase()))
            search.setName("%s(%d)".formatted(baseName, counter++));
    }
}