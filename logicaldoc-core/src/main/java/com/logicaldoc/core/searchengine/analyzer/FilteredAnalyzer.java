...
public class FilteredAnalyzer extends AnalyzerWrapper {
    ...
    protected TokenStreamComponents wrapComponents(String fieldName, TokenStreamComponents components) {
        ...
        for (String filter : order) {
            ...
            Map<String, String> filterSettings = config.getProperties("index.tokenfilter.%s.".formatted(filter));
            ...
            try {
                ...
                if (factory instanceof WordDelimiterGraphFilterFactory wdFactory) {
                    ...
                    ResourceLoader loader = new FilesystemResourceLoader(
                            new File("%s/logicaldoc/conf".formatted(Context.get().getConfig().getProperty("index.dir")))
                                    .toPath(),
    ...
    public static List<String> getTokenFilterNames(boolean justActives) {
        ...
        if (!justActives)
            ...
            for (String name : filters.keySet()) {
                if ("enabled".equals(config.getProperty("index.tokenfilter.%s".formatted(name))))
                    names.add(name);
            }
        }
        names.sort((String name1, String name2) -> {
            Integer pos1 = config.getInt("index.tokenfilter.%s.position".formatted(name1), 1);
            Integer pos2 = config.getInt("index.tokenfilter.%s.position".formatted(name2), 1);
