...
public class FolderTool {
    ...
    public String displayUrl(long tenantId, long folderId) {
        String url = StringUtil.ensureTrailingSlash(Context.get().getConfig().getProperty("server.url"));

        try {
            Tenant tenant = TenantDAO.get().findById(tenantId);
            url = "%sdisplay?tenant=%s&folderId=%d".formatted(url, tenant.getName(), folderId);
            return url;
