...
import com.logicaldoc.core.conversion.PopplerConverter;
...
public class ImageUtil {
    ...
    public static BufferedImage crop(BufferedImage originalImage, ImageZone zone) {
        ...
        if (zone.getLeft() <= 1 && zone.getTop() <= 1 && zone.getWidth() <= 1 && zone.getHeight() <= 1) {
            /*
             * The zone does not specify coordinates but values(0..1) relative
             * to the original image's size
             */
        ...
        if (inputExt.equals(outputExt)) {
            ...
            if (!"pdf".equals(inputExt)) {
                ...
                try {
                    ...
                    new PopplerConverter().print(pdfFile, out, 1);
                } finally {
                    FileUtil.delete(pdfFile);
                }
            } else {
                new PopplerConverter().print(originalFile, out, 1);
            }
        }
    }
