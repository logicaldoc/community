...
public abstract class Task implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (log.isErrorEnabled())
                log.info("Task {} completed in {}", getName(),
                        TimeDiff.printDuration(getScheduling().getLastDuration()));
    ...
    public void notifyReport() {
        ...
        while (st.hasMoreTokens()) {
            ...
            dictionary.put("task", I18N.message("task.name.%s".formatted(name), recipient.getLocale()));
    ...
    public void save() throws IOException, ParseException {
        ...
        props.setProperty("task.recipients.%s".formatted(name), getReportRecipients());
        props.setProperty("task.sendreport.%s".formatted(name), isSendActivityReport() ? "true" : "false");
        props.write();
    }
    ...
    @PostConstruct
    protected void init() {
        sendActivityReport = config.getBoolean("task.sendreport.%s".formatted(name));
        reportRecipients = config.getProperty("task.recipients.%s".formatted(name));
    }
}