...
public class ThumbnailManager {
    ...
    protected void createImage(Document document, String fileVersion, String type, String suffix, String sid)
        ...
        createImage(document, fileVersion,
                Context.get().getConfig().getInt("%s.gui.%s.size".formatted(tenantName, type), 200),
                Context.get().getConfig().getInt("%s.gui.%s.quality".formatted(tenantName, type), 93), suffix, sid);
    }
    ...
    private File writeToTempFile(Document document, String fileVersion) throws IOException, PersistenceException {
        File target = FileUtil.createTempFile("scr",
                ".%s".formatted(FileUtil.getExtension(DocUtil.getFileName(document, fileVersion))));
            ...
            try {
                ...
                if (!(builder instanceof ThumbnailBuilder))
                    throw new ClassNotFoundException(
                            "The specified builder %s doesn't implement ThumbnailBuilder interface"
                                    .formatted(className));
