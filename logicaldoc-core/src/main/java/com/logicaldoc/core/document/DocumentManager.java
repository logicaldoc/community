...
public class DocumentManager {
    ...
    public DocumentFuture replaceAlias(long aliasId, DocumentHistory transaction) throws PersistenceException {
        ...
        Document originalDoc = documentDAO.findById(alias.getDocRef(), true);
    ...
    public void promoteVersion(long docId, String version, DocumentHistory transaction)
        ...
        if (!document.isImmutable() && document.getStatus() == DocumentStatus.UNLOCKED) {
            Version ver = versionDAO.findByVersion(document.getId(), version, true);
            if (ver == null)
    ...
    private int getStore(Folder folder) {
        ...
        if (folder.getStore() != null)
            ...
            try {
                ...
                for (Folder parentFolder : folderDAO.initialize(parents)) {
