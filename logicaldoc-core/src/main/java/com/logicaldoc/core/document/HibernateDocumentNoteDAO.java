    ...
    public List<DocumentNote> findByDocIdAndTypes(long docId, long userId, String fileVersion, Collection<String> types)
        ...
        if (StringUtils.isEmpty(fileVersion)) {
            if (CollectionUtils.isEmpty(types)) {
                notes = findByWhere("_entity.docId = %d".formatted(docId), null, null);
            } else {
                notes = findByWhere("_entity.docId = :docId and _entity.type in (:types) and _entity.deleted = 0",
                        Map.of(DOC_ID, docId, "types", types), null, null);
            }
        } else if (CollectionUtils.isEmpty(types)) {
            notes = findByWhere("_entity.docId = :docId and _entity.fileVersion = :fileVersion and _entity.deleted = 0",
                    Map.of(DOC_ID, docId, "fileVersion", fileVersion), null, null);
        } else {
            notes = findByWhere(
                    "_entity.docId = :docId and _entity.fileVersion = :fileVersion and _entity.type in (:types) and _entity.deleted = 0",
