...
public class HibernateRatingDAO extends HibernatePersistentObjectDAO<Rating> implements RatingDAO {
    ...
    public void store(Rating rating, DocumentHistory transaction) throws PersistenceException {
        ...
        try {
            if (transaction != null) {
                ...
                transaction.setComment("rating: %d".formatted(rating.getVote()));
            }
    ...
    public Rating findVotesByDocId(long docId) throws PersistenceException {
         ...
         */
        String query = "select count(*), SUM(ld_vote) from ld_rating where ld_deleted=0 and ld_docid = %d"
                .formatted(docId);
    ...
    public Rating findByDocIdAndUserId(long docId, long userId) throws PersistenceException {
        List<Rating> results = findByWhere("_entity.docId = %d and _entity.userId = %d".formatted(docId, userId), null,
                null);
        if (results.isEmpty())
            return null;
        else
    ...
    public List<Rating> findByDocId(long docId) throws PersistenceException {
        return findByWhere("_entity.docId = %d".formatted(docId), "_entity.lastModified desc", null);
    }
    ...
    public void delete(long id, int code) throws PersistenceException {
        ...
        jdbcUpdate("delete from ld_rating where ld_id = %d".formatted(id));
