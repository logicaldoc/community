...
public class ThreadPools {
    ...
    public synchronized ThreadPoolExecutor getPool(String name) throws ThreadPoolNotAvailableException {
        ...
        if (pool != null) {
            ...
            int core = config.getInt("threadpool.%s.core".formatted(name), 5);
            int max = config.getInt("threadpool.%s.max".formatted(name), 10);
            int keepalive = config.getInt("threadpool.%s.keepalive".formatted(name), 5);
            String type = config.getString("threadpool.%s.type".formatted(name), TYPE_DEFAULT);
