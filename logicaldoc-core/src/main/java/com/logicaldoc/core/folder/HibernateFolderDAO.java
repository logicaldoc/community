...
public class HibernateFolderDAO extends HibernatePersistentObjectDAO<Folder> implements FolderDAO {
    ...
    private static final String ORACLE_DUPLE = "(%d,0)";
    ...
    private void workspaceChecks(Folder folder) throws PersistenceException {
        if (folder.getId() != 0L && folder.getType() == Folder.TYPE_WORKSPACE) {
            ...
            long defaultWorkspaceId = queryForLong(
                    ...
                    Map.of(ROOT_ID, root.getId(), "name", Folder.DEFAULTWORKSPACENAME));
    ...
    public List<Folder> findByUserId(long userId) throws PersistenceException {
        ...
        if (!userGroups.isEmpty()) {
            ...
            // policies
            folders = findByObjectQuery(
                    "select distinct(_folder) from Folder _folder left join _folder.accessControlList as _acl where _acl.ace.ace.groupId in (%s)"
                            .formatted(userGroups.stream().map(ug -> Long.toString(ug.getId()))
            ...
            if (folders.isEmpty()) {
                ...
                List<Folder> tmp = findByObjectQuery(
                        "select _folder from Folder _folder  where _folder.securityRef in (%s)".formatted(
                                folders.stream().map(f -> Long.toString(f.getId())).collect(Collectors.joining(","))),
                        null, null);
    ...
    public List<Folder> findChildren(long parentId, Integer max) throws PersistenceException {
        ...
        return findByWhere("_entity.parentId = :parentId and _entity.id != _entity.parentId", params, "_entity.name",
                max);
    }
    ...
    public boolean isReadAllowed(long folderId, long userId) throws PersistenceException {
        ...
        return queryForLong(
                "select distinct(ld_folderid) from ld_folder_acl where ld_read = 1 and ld_groupid in (%s) and ld_folderid = %d"
                        .formatted(
                                userGroups.stream().map(g -> Long.toString(g.getId())).collect(Collectors.joining(",")),
                                id)) > 0;
    }
    ...
    public List<Folder> findByGroupId(long groupId) throws PersistenceException {
        ...
        if (!coll.isEmpty()) {
            List<Folder> coll2 = findByObjectQuery(
                    "select _entity from Folder _entity where _entity.deleted = 0 and _entity.securityRef in (%s)"
                            .formatted(
                                    coll.stream().map(f -> Long.toString(f.getId())).collect(Collectors.joining(","))),
    ...
    public List<Long> findIdByUserId(long userId, long parentId) throws PersistenceException {
        ...
        if (!precoll.isEmpty()) {
             ...
             */
            List<Long> folderids2 = queryForList(
                    "select B.ld_id from ld_folder B where B.ld_deleted = 0 and B.ld_parentid = %d and B.ld_securityref in (%s)"
                            .formatted(parentId, query1),
                    Long.class);
    ...
    public List<Folder> findByName(Folder parent, String name, Long tenantId, boolean caseSensitive)
        ...
        if (caseSensitive)
            query.append("ld_name like '%s' ".formatted(SqlUtil.doubleQuotes(name)));
        else
            query.append("lower(ld_name) like '%s' ".formatted(SqlUtil.doubleQuotes(name.toLowerCase())));

        if (parent != null) {
            query.append(" and ld_parentid = %d ".formatted(parent.getId()));
            if (tenantId == null)
                query.append(" and ld_tenantid = %d ".formatted(parent.getTenantId()));
        }
    ...
    public List<Folder> findByNameAndParentId(String name, long parentId) throws PersistenceException {
        return findByWhere(
                "_entity.parentId = %d and _entity.name like '%s'".formatted(parentId, SqlUtil.doubleQuotes(name)),
                null, null);
    }
    ...
    public void restore(long folderId, long parentId, FolderHistory transaction) throws PersistenceException {
        ...
        int count = jdbcUpdate(
                "update ld_folder set ld_deleted = 0, ld_parentid = %d, ld_lastmodified = CURRENT_TIMESTAMP where not ld_type = %d and ld_id = %d"
                        .formatted(parent.getId(), Folder.TYPE_WORKSPACE, folderId));
        ...
        if (count == 0) {
            ...
            jdbcUpdate(
                    "update ld_folder set ld_deleted = 0, ld_parentid = %d, ld_lastmodified = CURRENT_TIMESTAMP where ld_type = %d and ld_id = %d"
                            .formatted(root.getId(), Folder.TYPE_WORKSPACE, folderId));
        }
        ...
        if (!treeIds.isEmpty()) {
            String idsStr = treeIds.stream().map(id -> Long.toString(id)).collect(Collectors.joining(","));
            jdbcUpdate(
                    "update ld_folder set ld_deleted = 0, ld_lastmodified = CURRENT_TIMESTAMP where ld_deleted = 1 and ld_id in (%s)"
                            .formatted(idsStr));
            jdbcUpdate(
                    "update ld_document set ld_deleted = 0, ld_lastmodified = CURRENT_TIMESTAMP where ld_deleted = 1 and ld_folderid in (%s)"
                            .formatted(idsStr));
        }
    }
    ...
    public Collection<Long> findFolderIdByUserIdInPath(long userId, long parentId) throws PersistenceException {
        ...
        if (!groupIds.isEmpty())
            query1.append(" and A.ld_groupid in (%s)"
                    .formatted(groupIds.stream().map(id -> Long.toString(id)).collect(Collectors.joining(","))));
    ...
    public Collection<Long> findFolderIdByUserIdAndPermission(
        ...
        if (user.isMemberOf(Group.GROUP_ADMIN) && parentId != null) {
            if (tree) {
                ...
                return queryForList(
                        "select ld_id from ld_folder where ld_deleted = 0 and (ld_id = %d or ld_parentid = %d) "
                                .formatted(parentId, parentId),
                        Long.class);
            }
        }
        ...
        if (permission != Permission.READ)
            query1.append(" and A.ld_%s = 1 ".formatted(permission.name().toLowerCase()));
        ...
        String masterIdsString = masterIds.stream().map(id -> Long.toString(id)).collect(Collectors.joining(","));

        /*
         * Now search for those folders that are or reference the masterIds
         */
        StringBuilder query2 = new StringBuilder("select B.ld_id from ld_folder B where B.ld_deleted = 0 ");
        query2.append(" and ( B.ld_id in (%s)".formatted(masterIdsString));
        query2.append(" or B.ld_securityref in (%s) ) ".formatted(masterIdsString));
    ...
    private void appendParentCondition(Long parentId, StringBuilder query, boolean tree) {
        if (parentId != null) {
            ...
            if (tree) {
                ...
                query.append(" (B.ld_id = %d or B.ld_parentId = %d) ".formatted(parentId, parentId));
            }
        }
    }
    ...
    private void appendUserGroupIdsCondition(User user, StringBuilder query) {
        ...
        if (!groupIds.isEmpty())
            query.append(" and A.ld_groupid in (%s)".formatted(groupIds.stream().collect(Collectors.joining(","))));
    }
    ...
    public void deleteAll(Collection<Folder> folders, int code, FolderHistory transaction) throws PersistenceException {
        for (Folder folder : folders) {
            ...
            try {
                ...
                log.warn("Error deleting folder %d, it may be normal".formatted(folder.getId()), e);
            }
        }
    }
    ...
    private void checkIfCanDelete(long folderId) throws PersistenceException {
        ...
        long rootId = findRoot(folder.getTenantId()).getId();
        if (folderId == rootId)
            throw new PersistenceException("You cannot delete folder %s - %d".formatted(folder.getName(), folderId));
    ...
    public void delete(long folderId, int delCode, FolderHistory transaction) throws PersistenceException {
        ...
        if (folder.getFoldRef() == null && folder.getType() != Folder.TYPE_ALIAS) {
            ...
            if (aliases != null && !aliases.isEmpty()) {
                ...
                int count = jdbcUpdate("update set ld_deleted = %d from ld_folder where ld_foldref in (%s)"
                        .formatted(delCode, aliasIds));
         ...
         */
        long updatedRows = jdbcUpdate(
                "update ld_folder set ld_securityref = null where ld_securityref = %d".formatted(folderId));
    ...
    public void applySecurityToTree(long rootId, FolderHistory transaction) throws PersistenceException {
        ...
        String treeIdsString = treeIds.stream().map(id -> Long.toString(id)).collect(Collectors.joining(","));
         ...
         */
        records = jdbcUpdate(
                "update ld_folder set ld_securityref = :securityRef, ld_lastmodified = :lastModified where not ld_id = :rootId and ld_id in (%s)"
                        .formatted(treeIdsString),
                Map.of("securityRef", securityRef, "lastModified", new Date(), ROOT_ID, rootId));
         ...
         */
        jdbcUpdate("delete from ld_folder_acl where not ld_folderid = :rootId and ld_folderid in (%s)"
                .formatted(treeIdsString), Map.of(ROOT_ID, rootId));
        ...
        if (sessionFactory.getCache() != null) {
            ...
            sessionFactory.getCache()
                    .evictCollectionData("%s.accessControlList".formatted(Folder.class.getCanonicalName()));
        }
    }
    ...
    private void setUniqueName(Folder folder) throws PersistenceException {
        ...
        List<String> collisions = queryForList(
                "select ld_name from ld_folder where ld_deleted = 0 and ld_parentid = %d and ld_name like'%s%%' and not ld_id = %d"
                        .formatted(folder.getParentId(), SqlUtil.doubleQuotes(folderName), folder.getId()),
                String.class);

        int counter = 1;
        while (collisions.contains(folder.getName()))
            folder.setName("%s(%d)".formatted(folderName, counter++));
    }
        ...
        if (REPLICATE.equals(securityOption)) {
            ...
            for (Long childId : childrenIds) {
                ...
                if (child.getSecurityRef() != null && isInPath(source.getId(), child.getSecurityRef())) {
                    ...
                    String copiedPathSecurityRef = "%s%s".formatted(newPath, relativeSourcePathSecurityRef);
    ...
    private Folder internalCopyValidation(
        ...
        if (!(securityOption == null || "none".equals(securityOption) || "inherit".equals(securityOption)
            ...
            throw new IllegalArgumentException("Invalid security option %s".formatted(securityOption));
        if (source == null)
    ...
    public void move(Folder source, Folder target, FolderHistory transaction) throws PersistenceException {
        ...
        // The new path
        String pathNew = "%s/%d".formatted(computePath(targetFolder.getId()), source.getId());
    ...
    public List<Folder> deleteTree(Folder folder, int delCode, FolderHistory transaction) throws PersistenceException {
        ...
        List<Long> ids = queryForList(
                "select ld_folderid from ld_document where ld_deleted=0 and ld_immutable = 1 and ld_folderid in (%s)"
                        .formatted(treeIdsString),
                Long.class);
        ...
        int records = jdbcUpdate(
                "update ld_folder set ld_deleted = %d where  ld_id in (%s)".formatted(delCode, treeIdsString));
        log.warn("Deleted {} folders in tree {} - {}", records, folder.getName(), folder.getId());

        jdbcUpdate("update ld_folder set ld_deleteuserid = %d where ld_id in (%s)".formatted(transaction.getUserId(),
                treeIdsString));
        jdbcUpdate("update ld_folder set ld_deleteuser = :user where ld_id in (%s)".formatted(treeIdsString),
        ...
        int aliases = jdbcUpdate(
                "update ld_folder set ld_deleted = %d where  ld_foldref in (%s)".formatted(delCode, treeIdsString));
        ...
        int documents = jdbcUpdate(
                "update ld_document set ld_deleted = %d where ld_folderid in (%s)".formatted(delCode, treeIdsString));
        log.warn("Deleted {} documents in tree {} - {}", documents, folder.getName(), folder.getId());

        jdbcUpdate("update ld_document set ld_deleteuserid = %d where  ld_folderid in (%s)"
                .formatted(transaction.getUserId(), treeIdsString));
        jdbcUpdate("update ld_document set ld_deleteuser=:user where  ld_folderid in (%s)".formatted(treeIdsString),
                Map.of("user", transaction.getUser().getFullName()));
    ...
    public Set<Long> findFolderIdInTree(long rootId, boolean includeDeleted) {
        ...
        while (!lastIds.isEmpty()) {
            ...
            String query = "select ld_id from ld_folder where %s %s"
                    .formatted(includeDeleted ? "" : " ld_deleted=0 and ", idsExpression);
    ...
    public Set<Long> findFolderIdInPath(long rootId, boolean includeDeleted) throws PersistenceException {
        ...
        ids = new HashSet<>(
                queryForList("select ld_id from ld_folder where ld_tenantid = %d %s and ld_path like '%s/%%'".formatted(
                        rootFolder.getTenantId(), includeDeleted ? "" : " and ld_deleted=0 ",
                        SqlUtil.doubleQuotes(rootFolder.getPath())), Long.class));
    ...
    public List<Folder> find(String name, Long tenantId) throws PersistenceException {
        return findByName(null, "%%%s%%".formatted(name), tenantId, false);
    }
    ...
    public List<Folder> findDeleted(long userId, Integer maxHits) {
        ...
        try {
            ...
            results = query(
                    "select ld_id, ld_name, ld_lastmodified, ld_color, ld_type from ld_folder where ld_deleted = 1 and ld_deleteuserid = %d"
                            .formatted(userId),
                    mapper, maxHits);
        } catch (Exception e) {
    ...
    public long countDocs(long folderId) {
        try {
            ...
            return queryForLong("SELECT COUNT(*) from ld_document WHERE ld_deleted = 0 and ld_folderid = %d"
    ...
    public List<Folder> findAliases(Long foldRef, long tenantId) {
        ...
        if (foldRef != null)
            query.append(" and _entity.foldRef = %d".formatted(foldRef));
    ...
    public List<Long> findFolderIdByTag(String tag) {
        try {
            return queryForList(
                    "select distinct(A.ld_folderid) from ld_foldertag A, ld_folder B where A.ld_folderid = B.ld_id and B.ld_deleted = 0 and lower(ld_tag) = '%s'"
                            .formatted(SqlUtil.doubleQuotes(tag).toLowerCase()),
                    Long.class);
        } catch (PersistenceException e) {
            log.error(e.getMessage(), e);
            return new ArrayList<>();
    ...
    private List<Long> findFolderIdByUserIdAndTag(long userId, String tag) throws PersistenceException {
        ...
        if (user.isMemberOf(Group.GROUP_ADMIN)) {
            ...
            StringBuilder query = new StringBuilder(
                    "select distinct(C.ld_id) from ld_folder C, ld_foldertag D where C.ld_id = D.ld_folderid AND C.ld_deleted = 0 ");
            ...
            query.append(" and D.ld_tag = '%s' ".formatted(SqlUtil.doubleQuotes(tag)));
    ...
    public List<String> findTags(long folderId) {
        try {
            return queryForList(
                    "select ld_tag from ld_foldertag where ld_folderid = %d order by ld_tag".formatted(folderId),
                    String.class);
        } catch (PersistenceException e) {
            log.error(e.getMessage(), e);
            return new ArrayList<>();
