...
public abstract class HibernatePersistentObjectDAO<T extends PersistentObject> implements PersistentObjectDAO<T> {
    ...
    public List<T> findByWhere(String where, Map<String, Object> parameters, String order, Integer max)
        ...
        try {
            String sorting = StringUtils.isNotEmpty(order) && !order.toLowerCase().contains("order by".trim())
                    ? " order by %s".formatted(order)
                    : order;
            StringBuilder query = new StringBuilder("from ");
            query.append(entityClass.getCanonicalName()).append(" _entity where _entity.deleted = 0 ");
            ...
            coll = findByObjectQuery(query.toString(), parameters, max);
            return coll;
        } catch (Exception e) {
            throw new PersistenceException(e);
        }
    ...
    public List<Long> findIdsByWhere(String where, Map<String, Object> parameters, String order, Integer max)
        ...
        try {
            String sorting = StringUtils.isNotEmpty(order) && !order.toLowerCase().contains("order by")
                    ? " order by %s".formatted(order)
                    : order;
            StringBuilder query = new StringBuilder("select _entity.id from ");
            query.append(entityClass.getCanonicalName());
            query.append(" _entity where _entity.deleted = 0 ");
            ...
            Query<Long> queryObject = prepareQuery(query.toString(), parameters, Long.class, max);
     ...
     */
    protected <R> Query<R> prepareQuery(
            String expression,
    ...
    public void deleteAll(Collection<T> entities, int code) throws PersistenceException {
        ...
        try {
            ...
            query.append(Integer.toString(code));
            query.append("  where id in ( ");
            query.append(entities.stream().map(e -> Long.toString(e.getId())).collect(Collectors.joining(",")));
            query.append(" )");

            prepareQuery(query.toString(), (Map<String, Object>) null, entityClass, null).executeUpdate();
        } catch (Exception e) {
            throw new PersistenceException(e);
        }
    }
    ...
    public int bulkUpdate(String expression, Map<String, Object> parameters) throws PersistenceException {
        ...
        try {
            MutationQuery mutationQuery = getCurrentSession()
                    .createMutationQuery("update %s %s".formatted(entityClass.getCanonicalName(), expression));
