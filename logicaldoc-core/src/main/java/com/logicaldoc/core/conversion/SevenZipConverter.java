 ...
 * @since 8.7.1
 */
public class SevenZipConverter extends AbstractFormatConverter {
	...
	private void convertSingleEntry(String sid, Document document, File src, File dest, String entry)
		...
		File uncompressedEntryFile = File.createTempFile("un7z", "." + entryExtension);
	...
	private void convertMultipleEntries(String sid, Document document, File src, File dest, List<String> entries)
			throws IOException {
		File tempFile = File.createTempFile("7zconvert", ".txt");
