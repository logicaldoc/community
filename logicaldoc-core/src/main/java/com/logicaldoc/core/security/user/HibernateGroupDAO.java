...
public class HibernateGroupDAO extends HibernatePersistentObjectDAO<Group> implements GroupDAO {
    ...
    public void delete(long groupId, int code) throws PersistenceException {
        ...
        group.setName("%s.%d".formatted(group.getName(), group.getId()));
    ...
    public void inheritACLs(Group group, long parentGroupId) throws PersistenceException {
        ...
        jdbcUpdate("delete from ld_menu_acl where ld_groupid = %d".formatted(groupId));

        log.debug("Delete all foldergroup for group {}", groupId);
        jdbcUpdate("delete from ld_folder_acl where ld_groupid = %d".formatted(groupId));

        log.debug("Delete all document ACL for group {}", groupId);
        jdbcUpdate("delete from ld_document_acl where ld_groupid = %d".formatted(groupId));
        ...
        jdbcUpdate(
                "insert into ld_menu_acl(ld_menuid, ld_groupid, ld_read, ld_write) select B.ld_menuid, %d, B.ld_read, B.ld_write from ld_menu_acl B where B.ld_groupid  = %d"
                        .formatted(groupId, parentGroupId));

        jdbcUpdate(
                """
                   insert into ld_folder_acl(ld_folderid, ld_groupid, ld_read, ld_preview, ld_write, ld_add, ld_security, ld_immutable, ld_delete, ld_rename, ld_import, ld_export, ld_sign, ld_archive, ld_workflow, ld_download, ld_calendar, ld_subscription, ld_print, ld_password, ld_move, ld_email, ld_automation, ld_store, ld_readingreq, ld_customid, ld_revision)
                   select B.ld_folderid, %d, B.ld_read, B.ld_preview, B.ld_write, B.ld_add, B.ld_security, B.ld_immutable, B.ld_delete, B.ld_rename, B.ld_import, B.ld_export, B.ld_sign, B.ld_archive, B.ld_workflow, B.ld_download, B.ld_calendar, B.ld_subscription, B.ld_print, B.ld_password, B.ld_move, B.ld_email, B.ld_automation, B.ld_store, B.ld_readingreq, B.ld_customid, B.ld_revision
                     from ld_folder_acl B
                    where B.ld_groupid = %d
                """.formatted(
                        groupId, parentGroupId));

        jdbcUpdate(
                """
                   insert into ld_document_acl(ld_docid, ld_groupid, ld_read, ld_preview, ld_write , ld_security, ld_immutable, ld_delete, ld_rename, ld_sign, ld_archive, ld_workflow, ld_download, ld_calendar, ld_subscription, ld_print, ld_password, ld_move, ld_email, ld_automation, ld_readingreq, ld_customid, ld_revision)
                   select B.ld_docid, %d, B.ld_read, B.ld_preview, B.ld_write, B.ld_security, B.ld_immutable, B.ld_delete, B.ld_rename, B.ld_sign, B.ld_archive, B.ld_workflow, B.ld_download, B.ld_calendar, B.ld_subscription, B.ld_print, B.ld_password, B.ld_move, B.ld_email, B.ld_automation, B.ld_readingreq, B.ld_customid, B.ld_revision
                     from ld_document_acl B
                    where B.ld_groupid = %d
                """.formatted(
                        groupId, parentGroupId));
    ...
    public Collection<Group> findByLikeName(String name, long tenantId) throws PersistenceException {
        return findByWhere("lower(_entity.name) like :name and _entity.tenantId = :tenantId",
                Map.of("name", name.toLowerCase(), "tenantId", tenantId), null, null);
    }
    ...
    public void fixGuestPermissions(Group group) throws PersistenceException {
        ...
        if (guest) {
            ...
            jdbcUpdate(
                    """
                    update ld_folder_acl set ld_write = 0, ld_add = 0, ld_security = 0, ld_immutable = 0, ld_delete = 0, ld_rename = 0, ld_import = 0, ld_export = 0,
            ...
            jdbcUpdate(
                    """
                    update ld_document_acl set ld_write = 0, ld_security = 0, ld_immutable = 0, ld_delete = 0, ld_rename = 0,
