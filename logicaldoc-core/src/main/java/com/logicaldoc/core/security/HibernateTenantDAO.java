...
public class HibernateTenantDAO extends HibernatePersistentObjectDAO<Tenant> implements TenantDAO {
    ...
    public Tenant findByName(String name) {
        Tenant tenant = null;
        try {
            Collection<Tenant> coll = findByWhere("_entity.name = :name", Map.of("name", name), null, null);
    ...
    public String getTenantName(long tenantId) throws PersistenceException {
        return queryForString(
                "select ld_name from ld_tenant where ld_deleted = 0 and ld_tenantid = %d".formatted(tenantId));
    }
}