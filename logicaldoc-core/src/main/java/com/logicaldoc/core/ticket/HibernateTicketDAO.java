...
public class HibernateTicketDAO extends HibernatePersistentObjectDAO<Ticket> implements TicketDAO {
	...
	public void deleteExpired() {
		...
		try {
			int count = jdbcUpdate("update ld_ticket set ld_deleted=1 where ld_deleted = 0 and ld_expired < :expired",
					Map.of("expired", new Date()));
			if (log.isDebugEnabled())
				log.debug("Deleted {} ticket expired before {}", count, new Date());
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	}
