...
public class DocumentServiceImplTest extends AbstractWPTestCase {
    ...
    @Test
    public void testCreateTicket() throws ServerException, PersistenceException {
        ...
        // exact ticket ID
        assertEquals("http://server:port/download-ticket?ticketId=%s".formatted(ticket.get(0)), ticket.get(1));
    ...
    public void testSaveAcl() throws Exception {
        GUIDocument doc = testSubject.getById(1L);
    ...
    public void testSave() throws Exception {
        ...
        att.setName("string-001");
        att.setValue("string2");
        att.setParent("string");
        doc.addAttribute(att);

        att = new GUIAttribute();
        att.setName("boolean");
        att.setIntValue(1L);
        att.setType(Attribute.TYPE_BOOLEAN);
        doc.addAttribute(att);
        att.setName("boolean2");
        att.setIntValue(null);
        att.setType(Attribute.TYPE_BOOLEAN);
        doc.addAttribute(att);
        ...
        att.setName("user");
        att.setIntValue(1L);
        att.setType(Attribute.TYPE_USER);
        doc.addAttribute(att);
        att = new GUIAttribute();
        att.setName("user2");
        att.setIntValue(null);
        ...
        att.setName("folder");
        att.setIntValue(4L);
        att.setType(Attribute.TYPE_FOLDER);
        doc.addAttribute(att);
        att = new GUIAttribute();
        att.setName("folder2");
        att.setIntValue(null);
        ...
        att = new GUIAttribute();
        att.setName("document");
        att.setIntValue(1L);
        att.setType(Attribute.TYPE_DOCUMENT);
        doc.addAttribute(att);
        att.setName("document2");
        att.setIntValue(null);
        att.setType(Attribute.TYPE_DOCUMENT);
        doc.addAttribute(att);
        ...
        att.setName("integer");
        att.setIntValue(1L);
        att.setType(Attribute.TYPE_INT);
        doc.addAttribute(att);
        att.setName("integer2");
        att.setIntValue(null);
        att.setType(Attribute.TYPE_INT);
        doc.addAttribute(att);
        ...
        att = new GUIAttribute();
        att.setName("double");
        att.setDoubleValue(1D);
        att.setType(Attribute.TYPE_DOUBLE);
        doc.addAttribute(att);
        att.setName("double2");
        att.setDoubleValue(null);
        att.setType(Attribute.TYPE_DOUBLE);
        doc.addAttribute(att);

        att = new GUIAttribute();
        att.setName("date");
        att.setDateValue(referenceInstant);
        ...
        // Add attributes but of different types than the template
    ...
    private void prepareTemplate() throws PersistenceException {
        ...
        template.setAttribute("string", attr);

        attr = new Attribute();
        attr.setType(Attribute.TYPE_BOOLEAN);
        template.setAttribute("boolean", attr);
        attr = new Attribute();
        attr.setType(Attribute.TYPE_BOOLEAN);
    ...
    @Test
    public void testSetPassword() throws ServerException, PersistenceException {
        Document doc = docDao.findById(1L);
        assertNotNull(doc);

        testSubject.setPassword(1L, "password");

        doc = docDao.findById(1L);
        assertNotNull(doc);
        assertNotNull(doc.getPassword());
        assertTrue(testSubject.isPasswordProtected(1L));

        assertFalse(testSubject.unprotect(1L, "xxx"));
        assertTrue(testSubject.unprotect(1L, "password"));
        doc = docDao.findById(1L);
        assertNotNull(doc);
        assertNotNull(doc.getPassword());
        assertTrue(testSubject.isPasswordProtected(1L));
        ...
        prepareSession("boss", "admin");
        try {
            testSubject.unsetPassword(1L, "xxx");
            fail("No exception in case of wrong password?");
        } catch (ServerException e) {
            // All ok
        }

        testSubject.unsetPassword(1L, "password");
        assertFalse(testSubject.isPasswordProtected(1L));
    }
    ...
    public void testDelete() throws ServerException, PersistenceException {
        ...
        testSubject.delete(List.of(1L, 2L, 3L, 6L));
    ...
    @Test
    public void testExtractEmail() throws ServerException, PersistenceException {
        ...
        doc.setFileName("attachments.eml");
        doc.setFolder(FolderDAO.get().findById(Folder.DEFAULTWORKSPACEID));

        DocumentHistory transaction = new DocumentHistory();
        transaction.setSession(session);
        doc = DocumentManager.get()
                .create(this.getClass().getResourceAsStream("/emailWithAttachments.eml"), doc, transaction)
                .getDocument();
        assertNotNull(doc);

        GUIEmail email = testSubject.extractEmail(doc.getId(), null);
        assertNotNull(email);
