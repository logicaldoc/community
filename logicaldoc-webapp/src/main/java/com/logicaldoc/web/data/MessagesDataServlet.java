...
public class MessagesDataServlet extends AbstractDataServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response, Session session, Integer max,
         ...
         */
        List<SystemMessage> unread = dao.findByRecipient(session.getUsername(), Message.Type.SYSTEM, 0);
        for (SystemMessage message : unread)
            printMessage(writer, message, false);

        List<SystemMessage> read = dao.findByRecipient(session.getUsername(), Message.Type.SYSTEM, 1);
