...
public class FolderHistoryDataServlet extends AbstractDataServlet {
    ...
    private void printHistory(PrintWriter writer, Object[] cols, Locale locale, boolean showSid) {
        ...
        writer.print(String.format("<id>%d</id>", SqlUtil.getColumnLongValue(cols[7])));
        ...
        writer.print(String.format("<date>%s</date>", df.format(SqlUtil.getColumnDateValue(cols[2]))));
        ...
        writer.print(String.format("<userId>%d</userId>", SqlUtil.getColumnLongValue(cols[12])));
