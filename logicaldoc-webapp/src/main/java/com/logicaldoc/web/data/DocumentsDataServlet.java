...
public class DocumentsDataServlet extends AbstractDataServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response, Session session, int max,
		...
		for (Document doc : documentRecords) {
			...
			writer.print(
					"<customId><![CDATA[" + (doc.getCustomId() != null ? doc.getCustomId() : "") + "]]></customId>");
			...
			writer.print("<created>" + (doc.getCreation() != null ? df.format(doc.getCreation()) : "") + "</created>");
			...
			writer.print("<tenantId>" + doc.getTenantId() + "</tenantId>");
			...
			writer.print("<comment><![CDATA[" + (doc.getComment() != null ? doc.getComment() : "") + "]]></comment>");
			writer.print("<workflowStatus><![CDATA[" + (doc.getWorkflowStatus() != null ? doc.getWorkflowStatus() : "")
					+ "]]></workflowStatus>");
			...
			if (StringUtils.isNotEmpty(doc.getTgs()))
				writer.print("<tags><![CDATA[" + doc.getTgs().substring(1, doc.getTgs().length() - 1) + "]]></tags>");
