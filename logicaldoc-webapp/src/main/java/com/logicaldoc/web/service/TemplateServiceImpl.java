...
public class TemplateServiceImpl extends AbstractRemoteService implements TemplateService {
    ...
    private Template getTemplate(GUITemplate guiTemplate, Session session, User sessionUser)
        ...
        if (guiTemplate.getId() != 0) {
            template = dao.findById(guiTemplate.getId(), true);
    ...
    public GUITemplate getTemplate(long templateId) throws ServerException {
        ...
        try {
            TemplateDAO dao = TemplateDAO.get();
            Template template = dao.findById(templateId, true);
            if (template == null)
                return null;
    ...
    private GUIAttribute toGuiAttribute(
            String attrName,
            Attribute templateExtAttr,
            Attribute setExtAttr,
            AttributeSet aSet) {
    ...
    private Template initializeTemplateAttributes(Template template) {
        if (template != null) {
            try {
                ...
                // not be loaded, so load the bean again and initialize it.
    ...
    private void addMultipleGuiAttributes(
    ...
    private void initializeExtensibleObjectValues(
            Template template,
    ...
    private Template loadExtensibleObjectTemplate(Template template, ExtensibleObject extensibleObject)
        ...
        if (extensibleObject instanceof Document) {
            ...
            FolderDAO foldDao = FolderDAO.get();
