...
public class MessageServiceImpl extends AbstractRemoteService implements MessageService {
    ...
    public List<GUIMessageTemplate> loadTemplates(String language, String type) throws ServerException {
        ...
        try {
            ...
            for (MessageTemplate test : standardTemplates) {
                ...
                t.setType(template.getType().name());
                t.setFooter(template.getFooter());
                t.setButtons(template.getButtons());
                buf.add(t);
            }
    ...
    private void storeTemplate(MessageTemplate template) throws ServerException {
        try {
            MessageTemplateDAO.get().store(template);
        } catch (Exception e) {
    ...
    public void deleteTemplates(String name) throws ServerException {
        ...
        try {
            ...
            for (MessageTemplate template : templates) {
                if (!template.getType().equals(MessageTemplate.Type.USER))
                    continue;
    ...
    public GUIMessageTemplate getTemplate(long templateId) throws ServerException {
        ...
        try {
            ...
            GUIMessageTemplate guiTemplate = new GUIMessageTemplate();
            guiTemplate.setId(template.getId());
            guiTemplate.setLanguage(template.getLanguage());
            guiTemplate.setName(template.getName());
            guiTemplate.setSubject(template.getSubject());
            guiTemplate.setBody(template.getBody());
            guiTemplate.setType(template.getType().name());
