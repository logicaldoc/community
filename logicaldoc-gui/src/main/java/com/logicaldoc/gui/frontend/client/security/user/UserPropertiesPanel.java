...
public class UserPropertiesPanel extends HLayout {
    ...
    public void prepareGUI() {
        ...
        TextItem whatsapp = prepareWhatsappItem(readonly);
        ...
        if (user.getId() == 0L)
            form1.setItems(notifyCredentials, guest, legals, username, email, firstname, name, email2, language,
                    timeZone, address, postalcode, city, country, state, phone, cell, whatsapp, company, department,
            ...
            form1.setItems(id, lastLogin, creation, username, notifyCredentials, guest, legals, email, firstname, name,
                    email2, language, timeZone, address, postalcode, city, country, state, phone, cell, whatsapp,
                    company, department, building, organizationalUnit);
        addMember(layout);
    ...
    private TextItem prepareWhatsappItem(boolean readonly) {
        TextItem whatsapp = ItemFactory.newTextItem("whatsapp", user.getWhatsapp());
        whatsapp.setDisabled(readonly);
        if (!readonly)
            whatsapp.addChangedHandler(changedHandler);
        return whatsapp;
    }
    ...
    boolean validate() {
        ...
        if (Boolean.FALSE.equals(vm.hasErrors())) {
            ...
            user.setWhatsapp((String) values.get("whatsapp"));
