...
/**
 * Shows the queue of documents to be processed by the Autofill task.
 * 
 * @author Marco Meschieri - LogicalDOC
 * @since 9.3
 */
public class AutofillQueuePanel extends VLayout {

    private RefreshableListGrid list;

    private int maxRecords = 10;

    public AutofillQueuePanel(int maxRecords) {
        ...
        reschedule.addClickHandler(event -> LD.ask(I18N.message("rescheduleallprocessing"),
                I18N.message("rescheduleallprocessingask"), answer -> {
                    if (Boolean.TRUE.equals(answer))
                        AutofillService.Instance.get().rescheduleAll(new DefaultAsyncCallback<>() {
    ...
    private void showContextMenu() {
        ...
        markUnprocessable.addClickHandler(event -> {
            ...
            AutofillService.Instance.get().markUnprocessable(GridUtil.getIds(selection), new DefaultAsyncCallback<>() {
                @Override
