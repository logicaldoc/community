...
public class WorkflowDashlet extends Portlet {
    ...
    public void onDraw() {
        ...
        list.addCellDoubleClickHandler(event -> workflowDashboard.openTaskDetails(
                event.getRecord().getAttributeAsString("id"), type == WorkflowDashboard.TASKS_INVOLVED));
        ...
        assign.addClickHandler(click -> {
            ...
            submit.addClickHandler(saveClick -> {
                if (user.getSelectedRecord() == null)
                    return;
                LD.contactingServer();
                WorkflowService.Instance.get().assignTasks(ids, user.getSelectedRecord().getAttributeAsLong("id"),
                        new DefaultAsyncCallback<>() {
