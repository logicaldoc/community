...
class MessageBox extends Label {
	...
	public MessageBox(String message, String role) {
		...
		avatarIcon = "<img src='" + url + "' style='border: 0px; height: 32px; width: 32px  margin-right: 5px; margin-bottom: 3px' />";
	...
	private String prepareContent(String message) {
		...
		return "<table border='0' width='100%'><tr><td style='width:36px; align:center; vertical-align: top;'>" + avatarIcon
				+ ("chatgpt".equals(role) ?  copyButton : "")
				+ "</td><td><span id='" + messageTextElementId + "'>" + message
				+ "</span></td></tr></table>";
	}
