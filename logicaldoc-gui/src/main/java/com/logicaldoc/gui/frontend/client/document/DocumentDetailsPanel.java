        ...
        if (document.getId() != 0)
            DocumentService.Instance.get().getAllowedPermissions(Arrays.asList(document.getId()),
                    new DefaultAsyncCallback<GUIAccessControlEntry>() {
                        ...
                        @Override
                        protected void handleSuccess(GUIAccessControlEntry result) {
                            document.setAllowedPermissions(result);
                            refresh();
                        }
                    });
        else
            FolderService.Instance.get().getAllowedPermissions(Arrays.asList(document.getFolder().getId()),
                    new DefaultAsyncCallback<GUIAccessControlEntry>() {
