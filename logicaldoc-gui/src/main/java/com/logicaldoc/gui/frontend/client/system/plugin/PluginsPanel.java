...
public class PluginsPanel extends VLayout {
    ...
    public void onDraw() {
        ...
        if (Session.get().getInfo().isCanUploadPackages())
            addMember(toolStrip);
        addMember(list);

        if (Session.get().isAdmin() && Session.get().isDefaultTenant() && !Session.get().isDemo() && Session.get().getInfo().isCanUploadPackages())
    ...
    private void showContextMenu() {
        ...
        initialize.addClickHandler(
                event -> LD.ask(I18N.message("question"), I18N.message("confirminitializeplugin"), value -> {
                    if (Boolean.TRUE.equals(value)) {
                        LD.contactingServer();
                        UpdateService.Instance.get().initializePlugin(
        ...
        uninstall.addClickHandler(
                event -> LD.ask(I18N.message("question"), I18N.message("confirmuninstallplugin"), value -> {
                    if (Boolean.TRUE.equals(value)) {
                        LD.contactingServer();
                        UpdateService.Instance.get().uninstallPlugin(
                                list.getSelectedRecord().getAttributeAsString("name"), new DefaultAsyncCallback<>() {
                                    @Override
                                    public void handleSuccess(Void result) {
