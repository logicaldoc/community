...
public class GUISettingsPanel extends AdminPanel {
	...
	private void initGUI(GUIParameter[] settings) {
		...
		for (GUIParameter p : settings) {
			...
			if (p.getName().endsWith("gui.notes.allowedit".toLowerCase()))
				allowNotesEditing.setValue(p.getValue().equals("true") ? "yes" : "no");
			...
			public void onClick(ClickEvent event) {
				...
				if (vm.validate()) {
					...
					params.add(new GUIParameter(Session.get().getTenantName() + ".gui.notes.allowedit",
							"yes".equals(values.get("allownotesediting")) ? "true" : "false"));
