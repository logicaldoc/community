...
/**
 * This panel shows the administration Access Control menu
 * 
 * @author Marco Meschieri - LogicalDOC
 * @since 6.0
 */
public class AccessControlMenu extends VLayout {

    public AccessControlMenu() {
        setMargin(10);
        setMembersMargin(5);
    ...
    private void addSecurityButton() {
        ...
        security.addClickHandler(click -> SecurityService.Instance.get().loadSettings(new DefaultAsyncCallback<>() {
            @Override
    ...
    private void addGroupsButton() {
        ...
        groups.setWidth100();
        groups.setHeight(25);
        groups.addClickHandler(click -> AdminScreen.get().setContent(new GroupsPanel()));

        if (Menu.enabled(Menu.GROUPS))
            addMember(groups);
    }
    ...
    private void addUsersButton() {
        ...
        users.setWidth100();
        users.setHeight(25);
        users.addClickHandler(click -> AdminScreen.get().setContent(new UsersPanel()));

        if (Menu.enabled(Menu.USERS))
            addMember(users);
    }
    ...
    private void addSamlButton() {
        ...
        saml.setWidth100();
        saml.setHeight(25);
        saml.addClickHandler(click -> AdminScreen.get().setContent(new SamlPanel()));
    ...
    private void addBruteForceButton() {
        ...
        bruteForcePrevention.addClickHandler(click -> AdminScreen.get().setContent(new BruteForcePanel()));
    ...
    private void addFirewallButton() {
        ...
        firewall.addClickHandler(click -> AdminScreen.get().setContent(new FirewallPanel()));
    ...
    private void addAntivirusButton() {
        ...
        antivirus.addClickHandler(click -> AdminScreen.get().setContent(new AntivirusPanel()));
        if (Feature.visible(Feature.ANTIVIRUS) && Menu.enabled(Menu.ANTIVIRUS)) {
            addMember(antivirus);
