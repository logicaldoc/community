...
public class ResponsesListPanel extends VLayout {
	...
	protected void onDraw() {
		...
		max.setStep(10);
		max.addChangedHandler((ChangedEvent event) -> refresh((Integer) max.getValue()));
		...
		export.addClickHandler((ClickEvent event) -> GridUtil.exportCSV(grid, true));

		ToolStripButton print = new ToolStripButton(I18N.message("print"));
		toolStrip.addButton(print);
		print.addClickHandler((ClickEvent event) -> GridUtil.print(grid));
		...
		grid.addDataArrivedHandler((DataArrivedEvent event) -> infoPanel
