...
public class FolderStandardPropertiesPanel extends FolderDetailTab {
    ...
    private void refresh() {
        ...
        final StaticTextItem size = ItemFactory.newStaticTextItem("size",
                folder.getStorage() > 0 ? Util.formatSize(folder.getStorage()) : "-");
    ...
    private SpinnerItem prepareMaxVersionsItem() {
        SpinnerItem versionsQuota = ItemFactory.newSpinnerItem("versionsquota", folder.getVersionsQuota());
        versionsQuota.setWrapTitle(false);
        versionsQuota.setDisabled(!folder.isWrite());
        boolean versionsQuotaVisible = folder.isWorkspace() && folder.getFoldRef() == null;
        versionsQuota.setVisible(versionsQuotaVisible);
        if (folder.isWrite() && versionsQuotaVisible)
            versionsQuota.addChangedHandler(changedHandler);
