	...
	public ContextMenu(final GUIFolder folder, final DocumentsGrid grid) {
		...
		rename.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
			public void onClick(MenuItemClickEvent event) {
				...
				LD.askForValue(I18N.message("rename"), I18N.message("filename"), selection[0].getFileName(),
						new ValueCallback() {
							@Override
							public void execute(final String value) {
								...
								if (!originalExtension.equalsIgnoreCase(Util.getExtension(newFilename))) {
									LD.ask(I18N.message("filename"), I18N.message("extchangewarn"),
											new BooleanCallback() {
						...
						if (value) {
							DocumentService.Instance.get().delete(ids, new AsyncCallback<Void>() {
								...
								public void onSuccess(Void result) {
									...
									// screen.
									if (grid.getFolder() != null && grid.getFolder().getId() == FolderController.get()
						...
						new AsyncCallback<Void>() {
							...
							public void onSuccess(Void result) {
								for (GUIDocument record : selection) {
									...
									if (DocumentController.get().getCurrentDocument() != null && DocumentController
											.get().getCurrentDocument().getId() == record.getId()) {
										DocumentController.get().getCurrentDocument()
												.setIndexed(Constants.INDEX_TO_INDEX);
										DocumentController.get()
												.modified(DocumentController.get().getCurrentDocument());
									} else {
										DocumentController.get().modified(record);
									}
								}
		...
		sign.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
			public void onClick(MenuItemClickEvent event) {
				DigitalSignatureDialog dialog = new DigitalSignatureDialog(selectionIds);
				dialog.show();
			}
		});
		...
		startWorkflow.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				...
				StartWorkflowDialog workflowDialog = new StartWorkflowDialog(selectionIds);
				workflowDialog.show();
			}
		});
