...
public class ItemFactory {
    ...
    public static DateItem newDateItemForAttribute(String name, String title) {
        ...
        DateItem item = newDateItem("_" + name.replace(" ", Constants.BLANK_PLACEHOLDER));
        item.setTitle(title != null && !title.isEmpty() ? title : name);
        return item;
    }
    ...
    public static FormItem newIntegerItemForAttribute(String name, String label, Integer value) {
        ...
        item.setTitle(label != null && !label.isEmpty() ? label : name);
    ...
    public static SelectItem newBooleanSelectorForAttribute(String name, String title, boolean allowEmpty) {
        ...
        map.put("0", I18N.message("no"));
        select.setValueMap(map);
        select.setTitle(title != null && !title.isEmpty() ? title : name);
        select.setWidth(80);
        select.setValue("");
    ...
    public static SelectItem newBarcodeTemplateSelector(
        ...
        return newBarcodeTemplateSelector(withEmpty, filterTemplateId, selectedTemplateId, false);
    }
    ...
    public static SelectItem newBarcodeTemplateSelector(
        ...
        templateItem.setWrapTitle(false);
        templateItem.setMultipleAppearance(MultipleAppearance.PICKLIST);
        templateItem.setOptionDataSource(new BarcodeTemplatesDS(withEmpty, filterTemplateId, all));
     ...
     * @return the new item
     */
    public static FormItem newFloatItemForAttribute(String name, String title, Float value) {
        // We cannot use spaces in items name
        String itemName = itemNameForAttribute(name);
        FormItem item = newFloatItem(itemName, title, value);
        item.setTitle(title != null && !title.isEmpty() ? title : name);
        if (!InputValues.getInputs(item.getName()).isEmpty()) {
            item = formItemWithSuggestions(item);
            IsFloatValidator iv = new IsFloatValidator();
