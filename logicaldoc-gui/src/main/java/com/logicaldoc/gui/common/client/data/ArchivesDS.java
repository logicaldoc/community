...
public class ArchivesDS extends DataSource {
    public ArchivesDS(int mode, Integer status, Long managerId) {
        setTitleField("name");
        ...
        setFields(id, name, description, size, closer, creator, typelabel, stat, mod, created, pathonserver);
        setClientOnly(true);
        setDataURL("data/archives.xml?mode=" + mode + "&locale=" + I18N.getLocale()
                + (status != null ? "&status=" + status : "") + (managerId != null ? "&managerId=" + managerId : ""));
    }
}