...
public abstract class Util {
    ...
    private static final String QUESTION_LOCALE_EQUAL = "?locale=";
    ...
    private static void redirectToRoot(String moduleName, String parameters) {
        ...
        url += QUESTION_LOCALE_EQUAL + I18N.getLocale() + "&tenant=" + Session.get().getTenantName();
        if (parameters != null)
            url += "&" + parameters;
        Util.download(url);
    }
    ...
    public static void redirectToSuccessUrl(String locale) {
        ...
        String queryString = WindowUtils.getRequestInfo().getQueryString();

        if (locale != null && !locale.isEmpty()) {
            if (queryString.contains("locale")) {
                queryString = queryString.replaceAll("locale=[a-zA-Z_]*", "locale=" + locale);
            } else if(queryString.isEmpty()) {
                queryString = QUESTION_LOCALE_EQUAL + locale;
            } else {
                queryString += AND_LOCALE_EQUAL + locale;
            }
        }

        WindowUtils.openUrl(url + queryString);
    }
