...
public abstract class LogicalDOCPlugin extends Plugin {
    ...
    private File getInstallMark() {
        ...
        return new File(resolveDataPath("install-%s".formatted(id)).toString());
    }
    ...
    protected void addScheduling(String taskName) {
        try {
            ...
            pbean.setProperty("schedule.cron.%s".formatted(taskName), "00 00 00 1 * ?");
            pbean.setProperty("schedule.length.%s".formatted(taskName), "-1");
            pbean.setProperty("schedule.enabled.%s".formatted(taskName), "true");
            pbean.setProperty("schedule.mode.%s".formatted(taskName), "simple");
            pbean.setProperty("schedule.interval.%s".formatted(taskName), "1800000");
            pbean.setProperty("schedule.delay.%s".formatted(taskName), "1800000");
            pbean.write();
        } catch (IOException e) {
