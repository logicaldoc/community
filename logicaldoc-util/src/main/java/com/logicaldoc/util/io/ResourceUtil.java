...
public class ResourceUtil {
    ...
    public static boolean existsResource(String path) {
        boolean exists = Thread.currentThread().getContextClassLoader().getResource(path) != null;
        if (!exists && path.startsWith("/"))
            exists = Thread.currentThread().getContextClassLoader().getResource(path.substring(1)) != null;
        return exists;
    }
