...
public class DropboxTestBench {
    ...
    public static void main(String[] args) throws IOException, DbxException, PersistenceException, EncryptionException {
        ...
        System.out.printf("%nentered %d".formatted(entered));
        System.out.println(client.getAccountName());

        List<Metadata> entries = client.list("/");
        for (Metadata entry : entries) {
            if (entry instanceof FolderMetadata)
                System.out.printf("%nFolder: %s %s", entry.getName(), entry.getPathDisplay());
            else
                System.out.printf("%nFile: %s %s", entry.getName(), entry.getPathDisplay());
        }
        ...
        client.downloadFile("/test/%s".formatted(file.getName()), file);
        client.uploadFile(file, "/test/%s".formatted(file.getName()));
    }

    public static String authorization()
            throws IOException, URISyntaxException, PersistenceException, EncryptionException {
        Dropbox client = new Dropbox(1L);

        // This is for authorizing the LogicalDOC application just the first
        // time
        String authUrl = client.startAuthorization(Locale.ENGLISH);
        System.out.printf("%n1. Go to: %s", authUrl);
        ...
        System.out.printf("%nYour access token is: %s", accessToken);
