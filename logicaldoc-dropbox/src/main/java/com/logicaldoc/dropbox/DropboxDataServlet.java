...
public class DropboxDataServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if ("#parent#".equals(parent)) {
                ...
                writer.print(String.format("<parent><![CDATA[%s]]></parent>", parent));
        ...
        if ((ent == null && "/".equals(parent)) || ent instanceof FolderMetadata) {
            ...
            for (Metadata entry : entries) {
                if (folders && entry instanceof FileMetadata)
                    continue;
                writer.print("<entry>");
                writer.print(String.format("<path><![CDATA[%s]]></path>", entry.getPathDisplay()));
                writer.print(String.format("<parent><![CDATA[%s]]></parent>", parent));
                writer.print(String.format("<name><![CDATA[%s]]></name>", entry.getName()));
                writer.print(String.format("<type>%s</type>", entry instanceof FileMetadata ? "file" : "folder"));
    ...
    private void printIcon(PrintWriter writer, Metadata entry) {
        if (entry instanceof FileMetadata)
            writer.print(String.format("<iicon>%s</iicon>", FileUtil.getBaseName(
